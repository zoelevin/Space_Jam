...
// Function for halfstep drive mode
...
// Function for Wave Drive mode

void WaveDriveMode(void) {
    switch (coilState) {
        case step_one:
            // coil drive both forward
            COIL_A_DIRECTION = 1;
...
void DRV881_drive(void) {
    switch (coilState) {
        case step_one:
            // coil drive both forward
            COIL_A_DIRECTION = 1; //step
            if (stepDir == FORWARD) {
                coilState = step_two;
            } else {
                coilState = step_one;
        ...
        case step_two:
            ...
            COIL_A_DIRECTION_INV = stepDir; //dir
    }
