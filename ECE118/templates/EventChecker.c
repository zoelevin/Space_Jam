...
#define DETECTED_THRESHOLD 1023
#define NOT_DETECTED_THRESHOLD 0

static enum {
    DETECTED, NOT_DETECTED
} lastTrack_WireState = NOT_DETECTED;

uint8_t Track_Wire_Detection(void) {

    enum {
        DETECTED, NOT_DETECTED
    } currentTrack_WireState;

    uint16_t currentTrack_WireValue;
    ...
    if (currentTrack_WireValue > NOT_DETECTED_THRESHOLD) {
        currentTrack_WireState = DETECTED;
        curEvent = TRACK_WIRE_DETECTED;

    }
    if (currentTrack_WireValue == NOT_DETECTED_THRESHOLD) {
        currentTrack_WireState = NOT_DETECTED;
        curEvent = TRACK_WIRE_NOT_DETECTED;


    }
    if (currentTrack_WireState != lastTrack_WireState) { //event detected
        ThisEvent.EventType = curEvent;
        ThisEvent.EventParam = (uint16_t) currentTrack_WireValue;
        returnVal = TRUE;
        lastTrack_WireState = currentTrack_WireState;
